...
### Этап 2: Изменение структуры — транспонирование (вкладка «Очистка_и_структура»)
...
- **Доля** (продажи менеджера ÷ общая выручка, знаменатель 
  закреплён знаком $): `=B2/$B$34`
- **Накопительный итог** (доля + накопленный итог строкой выше): `=C3+D2`
- **Категория ABC** (A — пока накопительный итог ≤ 80%, 
  B — до 95%, C — остальные): `=ЕСЛИ(D2<=0,8;"A";ЕСЛИ(D2<=0,95;"B";"C"))`
...
- **План выполнен** (≥ 300 000 ₽) (продажи ≥ целевых из справочника): `=ЕСЛИ(C10>=$E$3;"Да";"Нет")`
- **Бонус по грейдам** — проверка порогов сверху вниз: 
  ≥300 тыс. → 5% от продаж, ≥275 тыс. → 4%, ≥200 тыс. → 3%, 
  ≥150 тыс. → 2%, иначе 0: 
  `=ЕСЛИ(C10>=$A$6;C10*$B$6;ЕСЛИ(C10>=$A$5;C10*$B$5;ЕСЛИ(C10>=$A$4;C10*$B$4;ЕСЛИ(C10>=$A$3;C10*$B$3;0))))`
- **Премия** — оба условия сразу (продажи выше плана И 
  встречи ≥ 3) → 0,5% от продаж: 
  `=ЕСЛИ(И(C10>$E$3;НЕ(E10<$E$6));C10*$E$7;0)`
- **Активность** — хотя бы одно условие (продажи выше плана 
  ИЛИ звонки > 60) → 2% от продаж: 
  `=ЕСЛИ(ИЛИ(C10>$E$3;D10>$E$4);C10*$E$5;0)`

Знаки $ закрепляют ячейки справочников, чтобы формулы не 
съезжали при растягивании вниз.
...
1. **Топ-10 менеджеров (31% штата) приносят 49% выручки** — 
   3 573 065 ₽ из 7 257 035 ₽ (сумма продаж топ-10 ÷ общая 
   выручка)
